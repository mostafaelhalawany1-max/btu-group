...
import Head from 'next/head'; // 👈 ضروري جداً لربط ملف المانيفست
...
export default function Home() {
  ...
  useEffect(() => {
    // 1. التأكد من سماح المتصفح بالتثبيت
    window.addEventListener('beforeinstallprompt', (e) => {
      e.preventDefault();
      setDeferredPrompt(e);
      setShowInstallBanner(true);
      console.log("جاهز للتثبيت ✅");
    });

    // 2. إخفاء البانر لو التطبيق متثبت فعلاً
  ...
  return (
    <div style={{ fontFamily: 'Arial, sans-serif', direction: 'rtl', backgroundColor: '#f4f7f6', paddingBottom: showInstallBanner ? '100px' : '0' }}>
      
      {/* 🛠️ الجزء ده هو اللي كان ناقص عشان المتصفح يشوف التطبيق */}
      <head>
      ...
      {/* 1. 📢 شريط الأخبار المتحرك مع الكلمة الثابتة */}
      ...
      <header style={{ background: 'linear-gradient(rgba(26, 54, 93, 0.9), rgba(26, 54, 93, 0.9)), url("https://images.unsplash.com/photo-1581094288338-2314dddb7ee1?q=80&w=1000")', backgroundSize: 'cover', backgroundPosition: 'center', padding: '50px 20px', textAlign: 'center', color: 'white' }}>
        ...
        <div style={{ maxWidth: '500px', margin: '30px auto', position: 'relative', display: 'flex', alignItems: 'center' }}>
          <input 
            type="text" placeholder="ابحث عن خدمتك..." value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
            style={{ width: '100%', padding: '18px 60px 18px 25px', borderRadius: '35px', border: 'none', fontSize: '1.1rem', backgroundColor: '#FFFFFF', color: '#333', boxShadow: '0 8px 25px rgba(0,0,0,0.4)' }} 
          />
      ...
      {/* 3. قسم العروض والخدمات (الـ 3 كروت) */}
      ...
      {/* 🌟 شريط التثبيت (سيظهر حتماً الآن بإذن الله) */}
      {showInstallBanner && (
        <div style={{ position: 'fixed', bottom: '20px', left: '20px', right: '20px', backgroundColor: '#f37021', color: 'white', padding: '15px 25px', borderRadius: '50px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 1000, boxShadow: '0 10px 30px rgba(0,0,0,0.5)', border: '2px solid white' }}>
          <span style={{ fontWeight: 'bold' }}>📥 ثبت تطبيق BTU Group الآن</span>
          <button onClick={handleInstallClick} style={{ backgroundColor: 'white', color: '#f37021', border: 'none', padding: '10px 25px', borderRadius: '25px', fontWeight: 'bold', cursor: 'pointer' }}>تثبيت</button>
        </div>
      )}
